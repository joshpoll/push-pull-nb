...
export class Reactive<T> {
  ...
  setSignal(fnOrValue: T | ((prev: T) => T)): void {
    if (typeof fnOrValue === "function") {
      const fn = fnOrValue as (prev: T) => T;
      // console.log("setSignal", fn, this._value, fn(this._value));
...
type Setter<T> = (<U extends T>(value: (prev: T) => U) => void) &
  (<U extends T>(value: Exclude<U, Function>) => void) &
  (<U extends T>(value: Exclude<U, Function> | ((prev: T) => U)) => void);

export const createSignal = <T>(initialValue: T): [() => T, Setter<T>] => {
  const signal = new Reactive(initialValue);
  return [() => signal.get(), (value) => signal.setSignal(value)];
};
